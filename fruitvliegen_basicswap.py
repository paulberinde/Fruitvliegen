...
class BasicSwap(object):
    ...
    def searchPattern(self):
        ...
        If the subset is followed by incremental numbers, take the highest.
        Swap this pattern/group again after putting the highest number of the group at the position of the lowest.
        """
        for pattern in range(len(self.subset_initial)):
    ...
    def patternSwapBack(self, step):
        ...
        if (self.switcher == True):
            ...
            for pattern in range((step + self.patlen), (step - 1), -1):
