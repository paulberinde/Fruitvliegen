...
class FinalSwap(object):
    ...
    def getPatterns(self, genome_insert):
        """
        Compute a score for the number of patterns in the respective genome.
        """
        self.pattern = 0
        for index in range(0, 24):
            if ((genome_insert[index] == genome_insert[index + 1] + 1) or (genome_insert[index] == genome_insert[index + 1] - 1)):
                self.pattern += 1
    ...
    def runSimulation(self, times):
        ...
        for step in range(times):
            ...
            # create a list for the pattern-scores
            self.patternsChild = []
            
            # compute pattern-scores of every child and add the sum to a list
            for child in self.children:
                self.getPatterns(child)
                (self.patternsChild).append(self.pattern)
            
            # select the child with the highest pattern score
            chosenOne = self.children[(self.patternsChild).index(max(self.patternsChild))]
        ...
        print "Total steps:{}".format(prYellow(self.steps))
        print "Total transferred genes:{}".format(prYellow(self.genes))
...
objectFinalSwap.runSimulation(50)
